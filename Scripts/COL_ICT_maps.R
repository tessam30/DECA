...
ne_colombia <- ne_states(country = "colombia", returnclass = "sf") %>%
  mutate(
    iso_3166_2 = ifelse(iso_3166_2 == "CO-CUN" & name == "Bogota", "CO-DC", iso_3166_2),
...
world_chop <- world %>%
  ...
  filter(sovereignt %in% c("Panama", "Venezuela", "Ecuador", "Brazil", "Peru"))

# Admin 0 shape that is called at the end to make Colombia pop-out
...
oth_countries_cities <- terrain +
  geom_sf(data = world_chop, fill = "#d9d9d9", alpha = 0.35, size = 0.20, colour = "#252525") +
  geom_sf(
    data = ne_cities %>% filter(sov0name == "Colombia", str_detect(featurecla, "Admin-0*")),
    alpha = 0.10
  ) +
  geom_sf_text_repel(
    data = ne_cities %>% filter(sov0name == "Colombia", str_detect(featurecla, "Admin-0*")),
...
source <- str_c("Created by USAID GeoCenter on ", today(), "     |     Data source: National Administrative Department of Statistics of Colombia 2018")
...
# Function purpose: join the excel tables to the shapefile, reshape it long, and clean up vars; return a data.frame
# Filter parameter was goign to be used to make labeled maps, but scrapped
join_ict <- function(df, filter) {
  df_long <-
    ...
    mutate(
      label_first = ifelse(stat == filter, Departmento, NA_character_),
      value = formattable(value/100, digits = 2), # ensure our legends only have 2 digits
      stat = str_replace(stat, " \\(%\\)", ""), #escape and remove extra text
      stat = fct_reorder(stat, value, .desc = TRUE),
    ) # strip out the (%) at the end of stats
  return(df_long)
}

# Return a faceted map for each table; returns a ggplot object
...
map_plot <- function(df, vircolor = "B", title = "") {
  p <-
    ...
    scale_fill_viridis_c(option = vircolor, direction = -1, alpha = 0.75, label = scales::percent_format(), limits = c(0, 1)) +
    map_clean + theme(text = element_text(family = "SegoeUI"),
                      legend.text = element_text(size = 7)) +
    coord_sf(xlim = mapRange[c(1:2)], ylim = mapRange[c(3:4)]) +
    labs(
      x = "", y = "", title = str_c("**",title,"**"), # bolds title using the ggtext package
      caption = source,
...
# Bar graphs
bar_plot <- function(df, vircolor = "A", title = "") {
  df %>% 
    ...
    scale_fill_viridis_c(option = vircolor, direction = -1, label = scales::percent_format()) +
    scale_x_reordered() +
...
join_ict(ict$Table1, filter = "Televisión a color convencional") %>% 
  map_plot(., vircolor = "C", title = "Television owernship, by different types") %>%
  mapsave(., "COL_tv_ownership")



# Table 3 - Mobile phone ownership  ------
join_ict(ict$`Table 3`, filter = "Teléfono celular") %>%
  map_plot(., vircolor = "A", title = "Mobile phone ownership, by type") %>%
  mapsave(., "COL_mobile_ownership")
...
join_ict(ict$`Table 4`, filter = "Computador portátil") %>%
  ...
  mapsave(., "COL_computer_ownership")
...
join_ict(ict$`Table 5`, filter = "Hogares con Internet") %>%
  ...
  mapsave(., "COL_internet_connection")
...
join_ict(ict$`Table 11`, filter = "Teléfono celular") %>%
  ...
  mapsave(., "COL_internet_use_device")
...
join_ict(ict$`Table 12`, filter = "Redes Sociales") %>%
  ...
  mapsave(., "COL_internet_use_purpose")
